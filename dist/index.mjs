...
const runCopy = (outDir, options) => {
	...
	const allowList = new Set([...new Set(["main.js", "styles.css"]), ...options.include ?? []]);
	const buildFiles = fs.existsSync(outDir) ? fs.readdirSync(outDir).filter((f) => allowList.has(f)) : [];
